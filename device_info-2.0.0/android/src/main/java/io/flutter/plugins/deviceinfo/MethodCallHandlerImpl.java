...
class MethodCallHandlerImpl implements MethodChannel.MethodCallHandler {
  ...
  public void onMethodCall(MethodCall call, MethodChannel.Result result) {
    if (call.method.equals("getAndroidDeviceInfo")) {
      ...
      build.put("gsfId", getGsfAndroidId());
  ...
  private String getGsfAndroidId() {
    ...
    String[] params = {idKey};
    Cursor cur = contentResolver.query(uri, null, null, params, null);
    if (cur != null && (!cur.moveToFirst() || cur.getColumnCount() < 2)){
      if(!cur.isClosed())
        cur.close();
      return null;
    }
    ...
    try {
      if (cur != null) {
        String result =  Long.toHexString(Long.parseLong(cur.getString(1)));
        if(!cur.isClosed())
          cur.close();
        return result;
      }else {
